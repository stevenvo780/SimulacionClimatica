...
from data import fetch_reg_quality
...
def load_observations(start_year, end_year):
    cache_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "worldbank_reg_quality_usa.csv"
    )
    cache_path = os.path.abspath(cache_path)
    df, meta = fetch_reg_quality(
        cache_path,
        country="USA",
        indicator="RQ.EST",
...
def run():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte - Politicas Estrategicas (Jefe Final)\n\n")
