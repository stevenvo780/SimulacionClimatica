"""
validate.py — Políticas Estratégicas (WGI Regulatory Quality)
...
def load_real_data(start_date, end_date):
    ...
    cache_path = os.path.join(os.path.dirname(__file__), "..", "data", "wgi_rq.csv")
    df = fetch_reg_quality(int(start_date[:4]), int(end_date[:4]), cache_path=os.path.abspath(cache_path))
    df = df.rename(columns={df.columns[-1]: "value"}) if "value" not in df.columns else df
...
def main():
    config = CaseConfig(
        case_name="Políticas Estratégicas (WGI Regulatory Quality)",
        value_col="value",
        series_key="incidence",
        grid_size=20,
        persistence_window=4,
        synthetic_start="1996-01-01",
