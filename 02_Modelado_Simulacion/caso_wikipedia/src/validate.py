...
from data import fetch_wikipedia_monthly
...
def load_observations(start_date, end_date):
    cache_path = os.path.join(os.path.dirname(__file__), "..", "data", "wikipedia_pageviews_monthly.csv")
    cache_path = os.path.abspath(cache_path)
    df = fetch_wikipedia_monthly(start_date, end_date, cache_path=cache_path)
    df = df.dropna()
    return df
...
def make_synthetic_df(start_date, end_date, seed):
    ...
    obs = np.array(sim["w"]) + rng.normal(0.0, measurement_noise, size=steps)
...
def evaluate_phase(phase_name, df, start_date, end_date, split_date, synthetic_meta=None):
    ...
    seasonal_trend, trend_params = build_forcing(train_df, df, "attention_z")
    ...
    err_abm = rmse(abm["w"][val_start:], obs_val)
    err_ode = rmse(ode["w"][val_start:], obs_val)
    err_reduced = rmse(abm_reduced["w"][val_start:], obs_val)
    err_reduced_full = rmse(abm_reduced["w"][val_start:], abm["w"][val_start:])

    err_threshold = 0.6 * obs_std
    corr_abm = correlation(abm["w"][val_start:], obs_val)
    corr_ode = correlation(ode["w"][val_start:], obs_val)
    ...
    mean_delta = abs(mean(abm_pert["w"][val_start:]) - mean(abm["w"][val_start:]))
    var_delta = abs(variance(abm_pert["w"][val_start:]) - variance(abm["w"][val_start:]))
    c2 = mean_delta < 0.5 and var_delta < 0.5

    abm_rep = simulate_abm(eval_params, steps, seed=seeds["replication"])
    persistence_base = window_variance(abm["w"][val_start:], 12)
    persistence_rep = window_variance(abm_rep["w"][val_start:], 12)
    ...
    base_mean = mean(abm_base_no_assim["w"][val_start:])
    alt_mean = mean(abm_alt["w"][val_start:])
    ...
    persistence_ok = window_variance(abm["w"][val_start:], 12) < 1.5 * obs_persistence
    ...
    results = {
        ...
        "persistence": {
            "window_variance": window_variance(abm["w"][val_start:], 12),
...
def write_outputs(results):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte de Validacion - Caso Wikipedia\n\n")
        f.write("## Metadata\n")
        ...
        f.write("- Fase real: evaluacion final con datos Wikimedia Pageviews.\n")
