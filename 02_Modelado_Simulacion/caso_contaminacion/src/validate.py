...
def make_synthetic_df(start_date, end_date, seed):
    ...
    forcing_base = 0.0
    forcing_trend = 0.01
    forcing_seasonal_amp = 0.0
    forcing_seasonal_period = 1

    forcing = []
    for t in range(steps):
        forcing.append(forcing_base + forcing_trend * t)
    ...
    obs = np.array(sim["p"]) + rng.normal(0.0, measurement_noise, size=steps)
...
def build_forcing(train_df, full_df, value_col):
    ...
    return trend_full.tolist(), (slope, intercept)
...
def evaluate_phase(phase_name, df, start_date, end_date, split_date, synthetic_meta=None):
    ...
    expected_years = len(pd.date_range(start=start_date, end=end_date, freq="YS"))
    ...
    obs = df["pm25_z"].tolist()
    obs_val = val_df["pm25_z"].tolist()
    steps = len(obs)
    ...
    seasonal_trend, trend_params = build_forcing(train_df, df, "pm25_z")
    ...
    err_abm = rmse(abm["p"][val_start:], obs_val)
    err_ode = rmse(ode["p"][val_start:], obs_val)
    err_reduced = rmse(abm_reduced["p"][val_start:], obs_val)
    err_reduced_full = rmse(abm_reduced["p"][val_start:], abm["p"][val_start:])

    err_threshold = 0.6 * obs_std
    corr_abm = correlation(abm["p"][val_start:], obs_val)
    corr_ode = correlation(ode["p"][val_start:], obs_val)
    ...
    mean_delta = abs(mean(abm_pert["p"][val_start:]) - mean(abm["p"][val_start:]))
    var_delta = abs(variance(abm_pert["p"][val_start:]) - variance(abm["p"][val_start:]))
    c2 = mean_delta < 0.5 and var_delta < 0.5

    abm_rep = simulate_abm(eval_params, steps, seed=seeds["replication"])
    persistence_base = window_variance(abm["p"][val_start:], 5)
    persistence_rep = window_variance(abm_rep["p"][val_start:], 5)
    ...
    base_mean = mean(abm_base_no_assim["p"][val_start:])
    alt_mean = mean(abm_alt["p"][val_start:])
    ...
    for i in range(5):
        ...
        sensitivities.append(mean(s["p"][val_start:]))
    ...
    obs_persistence = window_variance(obs_val, 5)
    persistence_ok = window_variance(abm["p"][val_start:], 5) < 5.0 * obs_persistence
    ...
    results = {
        ...
        "persistence": {
            "window_variance": window_variance(abm["p"][val_start:], 5),
...
def write_phase_report(f, label, results):
    ...
    f.write(f"- expected_years: {results['data']['expected_years']}\n")
    f.write(f"- observed_years: {results['data']['observed_years']}\n")
...
def write_outputs(results):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte de Validacion - Caso Contaminacion PM2.5\n\n")
        f.write("## Metadata\n")
        ...
        f.write("- Fase real: evaluacion final con datos World Bank PM2.5 (WLD).\n")
