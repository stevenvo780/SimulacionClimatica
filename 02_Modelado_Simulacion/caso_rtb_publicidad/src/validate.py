...
def load_observations(start_date, end_date):
    cache_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "rtb_monthly.csv"
    )
    cache_path = os.path.abspath(cache_path)
    df = fetch_rtb_monthly(start_date, end_date, cache_path=cache_path)
    df = df.dropna()
    return df
...
def adversarial_forcing(obs, strength=0.25):
...
def run():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte - RTB Publicidad (Jefe Final)\n\n")
