...
from data import fetch_posttruth_monthly
...
def load_observations(start_date, end_date):
    cache_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "posttruth_wikipedia_monthly.csv"
    )
    cache_path = os.path.abspath(cache_path)
    df = fetch_posttruth_monthly(start_date, end_date, cache_path=cache_path)
    df = df.dropna()
    return df
...
def run():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte de Validacion - Caso Postverdad en Redes\n\n")
        ...
        for phase_key in ["synthetic", "real"]:
            ...
            for k, v in phase["data"].items():
                f.write(f"- {k}: {v}\n")
            f.write("\n### Auditoria de datos\n")
            f.write(f"- expected_months: {phase['data']['expected_months']}\n")
            f.write(f"- observed_months: {phase['data']['observed_months']}\n")
        ...
        f.write("- Fase real: evaluacion final con pageviews de Wikipedia.\n")
