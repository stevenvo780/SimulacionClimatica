...
from data import fetch_openalex_paradigms
...
def load_observations(start_year, end_year):
    cache_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "openalex_paradigms.csv"
    )
    cache_path = os.path.abspath(cache_path)
    df, meta = fetch_openalex_paradigms(cache_path, start_year=start_year, end_year=end_year)
    df = df.dropna()
    return df, meta
...
def run():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte de Validacion - Caso Paradigmas Cientificos\n\n")
        ...
        for phase_key in ["synthetic", "real"]:
            ...
            if phase_key == "real":
                ...
                if meta:
                    f.write("### Meta real\n")
                    for k, v in meta.get("concepts", {}).items():
                        f.write(f"- {k}_concept_id: {v.get('id')}\n")
                        f.write(f"- {k}_concept_name: {v.get('name')}\n")
                        f.write(f"- {k}_term: {v.get('term')}\n")
                    f.write("\n")
        ...
        f.write("- Fase real: evaluacion final con datos OpenAlex.\n")
