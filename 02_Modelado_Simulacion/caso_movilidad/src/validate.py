...
from data import fetch_mta_subway_monthly
...
def load_observations(start_date, end_date):
    cache_path = os.path.join(os.path.dirname(__file__), "..", "data", "mta_subway_monthly.csv")
    cache_path = os.path.abspath(cache_path)
    df = fetch_mta_subway_monthly(start_date, end_date, cache_path=cache_path)
    df = df.dropna()
    return df
...
def make_synthetic_df(start_date, end_date, seed):
    ...
    obs = np.array(sim["m"]) + rng.normal(0.0, measurement_noise, size=steps)

    df = pd.DataFrame({"date": dates, "mobility": obs})
    meta = {
...
def evaluate_phase(phase_name, df, start_date, end_date, split_date, synthetic_meta=None):
    ...
    obs = df["mobility_z"].tolist()
    obs_val = val_df["mobility_z"].tolist()
    steps = len(obs)
    ...
    seasonal_trend, trend_params = build_forcing(train_df, df, "mobility_z")
    ...
    err_abm = rmse(abm["m"][val_start:], obs_val)
    err_ode = rmse(ode["m"][val_start:], obs_val)
    err_reduced = rmse(abm_reduced["m"][val_start:], obs_val)
    err_reduced_full = rmse(abm_reduced["m"][val_start:], abm["m"][val_start:])

    err_threshold = 0.6 * max(obs_std, 0.3)
    corr_abm = correlation(abm["m"][val_start:], obs_val)
    corr_ode = correlation(ode["m"][val_start:], obs_val)
    c1 = err_abm < err_threshold and err_ode < err_threshold and corr_abm > 0.3 and corr_ode > 0.3
    ...
    mean_delta = abs(mean(abm_pert["m"][val_start:]) - mean(abm["m"][val_start:]))
    var_delta = abs(variance(abm_pert["m"][val_start:]) - variance(abm["m"][val_start:]))
    c2 = mean_delta < 0.5 and var_delta < 0.5

    abm_rep = simulate_abm(eval_params, steps, seed=seeds["replication"])
    persistence_base = window_variance(abm["m"][val_start:], 50)
    persistence_rep = window_variance(abm_rep["m"][val_start:], 50)
    ...
    base_mean = mean(abm_base_no_assim["m"][val_start:])
    alt_mean = mean(abm_alt["m"][val_start:])
    ...
    p["assimilation_strength"] = 0.0
        ...
        sensitivities.append(mean(s["m"][val_start:]))
    ...
    persistence_ok = window_variance(abm["m"][val_start:], 12) < 1.5 * obs_persistence
    ...
    results = {
        ...
        "persistence": {
            "window_variance": window_variance(abm["m"][val_start:], 12),
...
def write_outputs(results):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte de Validacion - Caso Movilidad Urbana\n\n")
        f.write("## Metadata\n")
        ...
        f.write("- Fase real: evaluacion final con datos MTA Subway (2020-2024).\n")
