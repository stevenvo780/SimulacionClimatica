...
def load_observations(start_date, end_date):
    cache_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "crypto_attention_daily.csv"
    )
    cache_path = os.path.abspath(cache_path)
    df = fetch_crypto_daily(start_date, end_date, cache_path=cache_path)
    df = df.dropna()
    return df
...
def make_synthetic_df(start_date, end_date, seed):
    ...
    series = np.sin(np.arange(steps) / 10.0) + 0.3 * np.sign(np.sin(np.arange(steps) / 90.0))

    measurement_noise = 0.05
    obs = series + rng.normal(0.0, measurement_noise, size=steps)

    df = pd.DataFrame({"date": dates, "attention": obs})
    meta = {"noise": measurement_noise}
    return df, meta
...
def calibrate_abm_params(obs, base_params, steps):
    ...
    for forcing_scale in [0.05, 0.1, 0.2]:
        for macro_coupling in [0.0, 0.1, 0.2]:
            for damping in [0.01, 0.05]:
...
def evaluate_phase(phase_name, df, start_date, end_date, split_date, synthetic_meta=None):
    ...
    coverage = observed_days / expected_days if expected_days else 0.0
    if coverage < 0.85:
    ...
    obs_persistence = window_variance(obs_val, 30)
    persistence_ok = window_variance(abm["incidence"][val_start:], 30) < 1.5 * obs_persistence
    ...
    results = {
        ...
        "persistence": {
            "window_variance": window_variance(abm["incidence"][val_start:], 30),
        ...
        "c4_validity": False,
        "c5_uncertainty": True,
        "overall_pass": False if not c1 else False,
    }
...
def run():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte de Falsacion - No-Estacionariedad\n\n")
        ...
        f.write("## Notas\n")
        f.write("- Caso diseñado para fallar por quiebres de regimen.\n")
