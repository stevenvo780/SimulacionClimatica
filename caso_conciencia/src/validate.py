...
from data import fetch_happiness_series
...
def load_observations(start_year, end_year):
    cache_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "owid_happiness.csv"
    )
    cache_path = os.path.abspath(cache_path)
    df, meta = fetch_happiness_series(
        cache_path,
        entity="World",
...
def evaluate_phase(phase_name, df, start_year, end_year, split_year, synthetic_meta=None, real_meta=None):
    ...
    persistence_base = window_variance(abm["incidence"][val_start:], 4)
    persistence_rep = window_variance(abm_rep["incidence"][val_start:], 4)
    ...
    obs_persistence = window_variance(obs_val, 4)
    persistence_ok = window_variance(abm["incidence"][val_start:], 4) < 1.5 * obs_persistence
    ...
    results = {
        ...
        "persistence": {
            "window_variance": window_variance(abm["incidence"][val_start:], 4),
...
def run():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte de Validacion - Caso Conciencia Fenomenica\n\n")
        ...
        for phase_key in ["synthetic", "real"]:
            ...
            if phase_key == "real":
                ...
                if meta:
                    ...
                    f.write(f"- entity: {meta.get('entity')}\n")
        ...
        f.write("- Fase real: evaluacion final con datos OWID (Cantril ladder).\n")
