"""
validate.py — Conciencia (Bienestar Global)
...
def load_real_data(start_date, end_date):
    ...
    cache_path = os.path.join(os.path.dirname(__file__), "..", "data", "owid_happiness.csv")
    df = fetch_happiness_series(int(start_date[:4]), int(end_date[:4]), cache_path=os.path.abspath(cache_path))
...
def main():
    config = CaseConfig(
        case_name="Conciencia (Bienestar Global)",
        value_col="value",
        series_key="incidence",
        grid_size=20,
        persistence_window=3,
        synthetic_start="2011-01-01",
