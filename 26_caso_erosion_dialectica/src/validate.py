"""
validate.py — Erosión Dialéctica del Discurso
...
from data import fetch_literacy_rate
from ode import simulate_ode
from hybrid_validator import CaseConfig, run_full_validation, write_outputs


def load_real_data(start_date, end_date):
    cache_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "wb_literacy_rate.csv"))
    df, _ = fetch_literacy_rate(cache_path, start_year=int(start_date[:4]), end_year=int(end_date[:4]))
...
def main():
    config = CaseConfig(
        case_name="Erosión Dialéctica del Discurso",
        value_col="value",
        series_key="ed",
        grid_size=20,
