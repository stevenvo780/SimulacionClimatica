"""
validate.py — Justicia (Rule of Law WGI)
...
def load_real_data(start_date, end_date):
    ...
    cache_path = os.path.join(os.path.dirname(__file__), "..", "data", "wgi_rl.csv")
    df = fetch_rule_of_law(int(start_date[:4]), int(end_date[:4]), cache_path=os.path.abspath(cache_path))
...
def main():
    config = CaseConfig(
        case_name="Justicia (Rule of Law WGI)",
        value_col="value",
