...
from data import fetch_rule_of_law
...
def load_observations(start_year, end_year):
    cache_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "worldbank_rule_of_law_usa.csv"
    )
    cache_path = os.path.abspath(cache_path)
    df, meta = fetch_rule_of_law(
        cache_path,
        country="USA",
        indicator="RL.EST",
...
def evaluate_phase(phase_name, df, start_year, end_year, split_year, synthetic_meta=None, real_meta=None):
    ...
    train_mean = float(np.mean(train_df["value"].tolist()))
    train_std = float(np.std(train_df["value"].tolist())) if len(train_df) > 0 else 0.0
    if train_std > 0.0:
    ...
    persistence_base = window_variance(abm["incidence"][val_start:], 5)
    persistence_rep = window_variance(abm_rep["incidence"][val_start:], 5)
    ...
    persistence_ok = window_variance(abm["incidence"][val_start:], 5) < 1.5 * obs_persistence
    ...
    results = {
        ...
        "persistence": {
            "window_variance": window_variance(abm["incidence"][val_start:], 5),
...
def run():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte de Validacion - Caso Justicia y Normatividad\n\n")
        ...
        for phase_key in ["synthetic", "real"]:
            ...
            if phase_key == "real":
                ...
                if meta:
                    ...
                    f.write(f"- country: {meta.get('country')}\n")
                    f.write(f"- indicator: {meta.get('indicator')}\n")
        ...
        f.write("- Fase real: evaluacion final con datos WGI (Rule of Law).\n")
