"""
validate.py — Paradigmas Científicos (OpenAlex)
...
def load_real_data(start_date, end_date):
    ...
    cache_path = os.path.join(os.path.dirname(__file__), "..", "data", "openalex_paradigms.csv")
    df = fetch_openalex_paradigms(int(start_date[:4]), int(end_date[:4]), cache_path=os.path.abspath(cache_path))
...
def main():
    config = CaseConfig(
        case_name="Paradigmas Científicos (OpenAlex)",
        value_col="value",
        series_key="incidence",
        grid_size=20,
        persistence_window=10,
        synthetic_start="1950-01-01",
        synthetic_end="2023-01-01",
        synthetic_split="1990-01-01",
        real_start="1950-01-01",
        real_end="2023-01-01",
        real_split="1990-01-01",
        corr_threshold=0.7,
        extra_base_params={},
    )
