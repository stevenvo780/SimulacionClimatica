"""
validate.py — Movilidad Urbana (MTA NYC)
...
def load_real_data(start_date, end_date):
    ...
    cache_path = os.path.join(os.path.dirname(__file__), "..", "data", "mta_subway.csv")
    df = fetch_mta_subway_monthly(start_date, end_date, cache_path=os.path.abspath(cache_path))
    df = df.rename(columns={"mobility": "value"})
...
def main():
    config = CaseConfig(
        case_name="Movilidad Urbana (MTA NYC)",
        value_col="value",
        series_key="m",
        grid_size=10,
        ...
        corr_threshold=0.3,
        extra_base_params={"flow_scale": 0.05},
    )
