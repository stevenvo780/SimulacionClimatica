...
def load_observations(start_date, end_date):
    cache_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "memetic_daily.csv"
    )
    cache_path = os.path.abspath(cache_path)
    df = fetch_memetic_daily(start_date, end_date, cache_path=cache_path)
    df = df.dropna()
    return df
...
def evaluate_phase(phase_name, df, start_date, end_date, split_date, synthetic_meta=None):
    ...
    persistence_base = window_variance(abm["incidence"][val_start:], 14)
    persistence_rep = window_variance(abm_rep["incidence"][val_start:], 14)
    ...
    obs_persistence = window_variance(obs_val, 14)
    persistence_ok = window_variance(abm["incidence"][val_start:], 14) < 1.5 * obs_persistence
    ...
    results = {
        ...
        "persistence": {
            "window_variance": window_variance(abm["incidence"][val_start:], 14),
...
def run():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Reporte de Falsacion - Exogeneidad y Shock\n\n")
        ...
        f.write("## Notas\n")
        f.write("- Caso diseñado para fallar sin nudging.\n")
